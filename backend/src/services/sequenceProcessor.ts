...
import { sendSequenceEmail } from './emailService.js';
...
async function processEnrollment(enrollment: any): Promise<void> {
  ...
  const existingEmail = await query(
    `SELECT id FROM email_logs
     WHERE sequence_id = $1 AND member_id = $2 AND step_number = $3`,
    [enrollment.sequence_id, enrollment.member_id, currentStep]
  );

  if (existingEmail.rows.length > 0) {
    // Already sent, advance to next step
    await query(
      `UPDATE member_enrollments SET current_step = $1, updated_date = CURRENT_TIMESTAMP WHERE id = $2`,
      [currentStep + 1, enrollment.enrollment_id]
    );
    return;
  }
    ...
    if (nextStep >= steps.length) {
      await query(
        `UPDATE member_enrollments
         ...
         WHERE id = $2`,
        [nextStep, enrollment.enrollment_id]
      );
    } else {
      await query(
        `UPDATE member_enrollments SET current_step = $1, updated_date = CURRENT_TIMESTAMP WHERE id = $2`,
...
export async function enrollMemberInOnboarding(memberId: string): Promise<void> {
  try {
    // Get active onboarding sequences
    const sequencesResult = await query(
      'SELECT id FROM email_sequences WHERE is_onboarding = true AND is_active = true'
    );
    ...
    for (const sequence of sequencesResult.rows) {
      ...
      const existing = await query(
        `SELECT id FROM member_enrollments
         WHERE member_id = $1 AND sequence_id = $2 AND status IN ('active', 'completed')`,
        [memberId, sequence.id]
      );

      if (existing.rows.length === 0) {
        // Enroll in sequence
        await query(
          `INSERT INTO member_enrollments (member_id, sequence_id, current_step, status)
           VALUES ($1, $2, 0, 'active')`,
          [memberId, sequence.id]
        );
...
export function startSequenceProcessor(): void {
  // Run immediately
  processSequences();
